...
make
make test
sudo bash -c "echo 'cd $HOME/redis-stable' > /usr/local/bin/StartRedis.sh"
...
sudo bash -c "echo 'cd $HOME/redis-stable'  > /usr/local/bin/StopRedis.sh"
...
sed -i 's/# requirepass foobared/requirepass geheim/g' $HOME/redis-stable/redis.conf
